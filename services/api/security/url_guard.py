"""Lightweight SSRF guard for user-submitted URLs (§19).

Ported from MeTube's url_guard.py (AGPL-3.0) with attribution.
...
log = logging.getLogger("m2p.security.url_guard")
...
def _address_allowed_at_connect(addr: str, allow_loopback: bool = False) -> bool:
    ...
    host:port of an operator-configured proxy, never as a blanket rule.
    """
    ip = _normalise_ip(addr)
    if ip is None:
        return False
...
def _proxy_endpoint(proxy_url: str):
    """Parse a proxy URL into a ``(hostname, port)`` pair, or ``None``."""
...
def _collect_proxy_endpoints(proxy_urls) -> set:
    """Endpoints of every proxy this download may legitimately dial."""
...
def _loopback_endpoint_allowed(host, port) -> bool:
    ...
    return (
        ...
        _normalise_port(port),
    ) in _allowed_loopback_endpoints
...
def _guarded_getaddrinfo(host, *args, **kwargs):
    ...
    allowed = [
        r for r in results if _address_allowed_at_connect(r[4][0], allow_loopback)
    ]
    if not allowed:
        raise socket.gaierror(
            f"Refusing to connect to non-global address for host {host!r}"
        )
    return allowed
...
def install_socket_guard(allow_private: bool = False, proxy_urls=()) -> None:
    ...
    *proxy_urls* are the operator's configured proxies. A proxy on loopback is
    reachable at its own host:port, and nothing else on loopback is.
    ...
    for host, port in sorted(
        _allowed_loopback_endpoints, key=lambda ep: (ep[0], ep[1] or 0)
    ):
